...
namespace MouseThingy
{
    public class MouseHooker
    {
        ...
        private static bool _bothWasDown = false;

        static Point _prevpoint = null;
        public IntPtr CallbackFunction(int code, IntPtr wParam, IntPtr lParam)
        {
            //Console.WriteLine(code);
            var test = User32Import.CallNextHookEx(IntPtr.Zero, code, wParam, lParam);
            if (code < 0)
            {
            ...
            if (code == 0)
            {
                ...
                switch (wParamAsInt)
                {
                    ...
                    case "202":
                        ...
                        Console.WriteLine("right up" + _rightDown);
                        _leftDown = false;
                        _prevpoint = null;
                    ...
                    case "204":
                        ...
                        if (KeyboardHooker.LeftCtrDown)
                        {
                            ...
                            _groups.RemoveWindowFromGroup(_groups.GetCurrentControlGroup(),
                                new WindowControlGroups.Window("".ToString(), rootWindow, rect));
